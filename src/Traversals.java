...
public class Traversals {
  ...
  public static int countInternalNodes(TreeNode<Integer> node) {
    ...
    */
    ...
    // int leftTree = countInternalNodes(node.left);
    // int rightTree = countInternalNodes(node.right);
    // if (node.left != null || node.right != null) { return + leftTree + rightTree + 1; }

    return (node.left != null || node.right != null) ? countInternalNodes(node.left) + countInternalNodes(node.right) + 1 : 0;
  }
  ...
  public static <T> List<T> collectLevelOrderValues(TreeNode<T> node) {
     /*
         ...
         * "D#4"  "E9"
         *
         * Level order: Alpha, Beta7, Gamma1, D#4, E9 actual [Alpha, Beta7, D#4, E9, Gamma1]
         */
    if (node == null) { return List.of(); }
    List<T> allLevelOrders = new ArrayList<>();

    
    allLevelOrders.addAll(collectLevelOrderValues(node.left));
    allLevelOrders.add(node.value);
    allLevelOrders.addAll(collectLevelOrderValues(node.right));
    return allLevelOrders;
  }
  ...
  public static int countDistinctValues(TreeNode<Integer> node) {
    ...
    countDistinctValuesHelper(node, nums);
  
    return nums.size();
  }

  public static void countDistinctValuesHelper(TreeNode<Integer> node, HashSet<Integer> set){
    if(node == null) return;

    set.add(node.value);

    countDistinctValuesHelper(node.left, set);
    countDistinctValuesHelper(node.right, set);
  } 
  ...
  public static boolean hasStrictlyIncreasingPath(TreeNode<Integer> node) {
    ...
    return hasStrictlyIncreasingPathHelper(node, stack);
  }
  public static boolean hasStrictlyIncreasingPathHelper(TreeNode<Integer> node, Stack<Integer> stack) {
    if (node == null) return true;
    if (node.left == null || node.right == null) return true;
    
    if (!stack.isEmpty()) {
      int previous = stack.peek();
      if (previous > node.left.value && previous > node.right.value) return false;
    } else {
      stack.add(node.value);
    }

    return (hasStrictlyIncreasingPathHelper(node.left, stack) && hasStrictlyIncreasingPathHelper(node.right, stack));
